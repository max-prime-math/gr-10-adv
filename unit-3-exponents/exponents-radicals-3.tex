...
\end{tasks}
\prb{
In your own words, explain the following terms (based on how they are used in this module).
\begin{tasks}(2)
\task domain
\task relation
\end{tasks}
You may want to include definitions for these items as well as for range and function on your Resource Sheet.
}
\prb{
...
\prb{\begin{aligned}
&\text { Given the following ordered pairs, state the domain and range of the relation. }\\
&\{(1,4)(2,6)(3,8)(4,10)\}
\end{aligned}}
...
\prb{Decide whether the relations shown are one-to-one. Justify.\\[4pt]
    \begin{tasks}(2)
        \task
\mappingdiagram{1,2,3}{a,b,c}{1/1,2/2,3/3}
\task
...
\prb{The diagram below is not one-to-one. What is the \emph{fewest} number of domain elements you must remove to make it one-to-one? Give one possible choice.\\[4pt]
\centering
